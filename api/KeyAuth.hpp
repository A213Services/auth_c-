...
namespace KeyAuth {
	...
	public:
		...
		void upgrade(std::string username, std::string key) {
			...
			if (json[("success")])
			{
				...
				std::cout << XorStr("\n\n be careful your key to connect remains the first key when you register even after the upgrade with your new key so please keep your 1st key !");
				Sleep(10000);
				exit(0);
			}
			else
			{
				std::cout << XorStr("\n\n Upgrade failed one of your license is incorrect please contact your seller !");
				std::cout << std::string(json[("message")]);
				Sleep(3500);
				exit(0);
			}
	...
	private:
		...
		static std::string req(std::string data) {
			...
			// URL API NOT CHANGE !
			curl_easy_setopt(curl, CURLOPT_URL, XorStr("https://panel.a213.fr/api/0.0/").c_str());
